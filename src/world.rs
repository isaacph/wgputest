...
impl World {
    pub fn new() -> Self {
        ...
        //     }
        // }).collect::<Vec<_>>();
        let color_placeholder = cgmath::Vector4::new(0.0, 0.0, 0.0, 0.0);
        let ceiling = StageObject::new(
            BoundingBox::new(0, 1, 1, 1),
            cgmath::Vector2::new(1.5, 0.9),
            color_placeholder
        );
        // TODO: add a gameobject that responds to input
        let mut objects: Vec<Box<GameObject> > = Vec::new();
        let objects = vec![
            StageObject::new(
                BoundingBox::new(0, 1, 1, 1),
                cgmath::Vector2::new(1.5, 0.9),
